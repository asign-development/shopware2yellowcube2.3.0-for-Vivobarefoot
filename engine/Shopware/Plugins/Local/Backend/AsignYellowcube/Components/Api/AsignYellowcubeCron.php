        ...
        if (count($aOrders) > 0) {
            foreach ($aOrders as $order) {
                try {
                    ...
                    // check if the Status in the Order table for the fields:
                    // 1. ycResponse
                    // 2. ycWabResponse
                    // 3. ycWarResponse
                    $sRequestField = $this->getOrderRequestField($iOrdid);

                    // Now Considering all three fields for the orders are filled. Then the
                    // variable $sRequestField will contain an empty response from the function above.
                    // And if all three fields are filled then that means that order is succesfully done
                    // and we do not need to do anything further and we will continue the iteration
                    // for the next order.
                    if (isset($sRequestField) && $sRequestField != '') {
                    ...
                    if (($iStatusCode == null || $iStatusCode == 101) && $this->objOrders->getFieldData($iOrdid, $sRequestField) == '') {
                        ...
                        // only incrementing the counter so that we can log how many orders were created
            ...
            foreach ($aArticles as $article) {
                ...
                try {
                    ...
                    if ($iStatusCode === null) {
                        ...
                        $oRequestData = json_decode(json_encode($oRequestData), true);
