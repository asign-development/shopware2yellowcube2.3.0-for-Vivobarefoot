...
class Repository extends ModelRepository {
	...
	public function getOrdersListQuery( $filters, $sort, $offset = 0, $limit = 100 ) {
		$sOrdersColumns  = [
			...
			'amountNet'   => 'invoice_amount_net',
			'timestamp'   => 'ordertime',
			'userID'
		];
		$sYcubeOrderCols = [ 'id', 'lastSent', 'ycReference', 'ycResponse', 'ycWabResponse', 'ycWarResponse' ];
		$select          = Shopware()->Db()->select()
		                             ->from( 's_order', $sOrdersColumns )
		                             ->joinLeft( 's_core_paymentmeans', 's_order.paymentID = s_core_paymentmeans.id', [ 'payment' => 'description' ] )
		                             ->joinLeft( 's_premium_dispatch', 's_order.dispatchID = s_premium_dispatch.id', [ 'shipping' => 'name' ] )
		                             ->joinLeft( 's_core_states', 's_order.status = s_core_states.id', [ 'status' => 'description' ] )
		...
		if ( $filters ) {
			foreach ( $filters as $filter ) {
				...
				$select->orWhere( 'asign_yellowcube_orders.ycReference = ?', $filter["value"] );
				$select->orWhere( 's_core_states.description LIKE ?', '%' . $filter["value"] . '%' );
			}
		}
