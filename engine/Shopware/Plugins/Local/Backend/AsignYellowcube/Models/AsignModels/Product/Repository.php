...
class Repository extends ModelRepository {
	 ...
	 * @return array
	 */
	public function getProductListData( $filters, $sort, $offset = 0, $limit = 100 ) {
		$select = $this->getProductsListQuery( $filters, $sort, $offset, $limit );
		$result = [];
	...
	public function getProductsListQuery( $filters, $sort, $offset = 0, $limit = 100 ) {
		$select = Shopware()->Db()->select()
		                    ->from( 's_articles', [ 'artid' => 'id', 'name', 'active', 'tara', 'tariff', 'origin' ] )
		                    ->joinLeft( 's_articles_attributes', 's_articles.id = s_articles_attributes.articleID AND yc_export = 1', [ 'yc_export' ] )
		                    ->joinLeft( 's_articles_details', 's_articles.id = s_articles_details.articleID', [
			                    'id',
			                    'ordernumber',
			                    'ean',
			                    'instock',
		                    ...
		                    ->joinLeft( 's_articles_esd', 's_articles.id = s_articles_esd.articleID', [ 'esdid' => 'id' ] )
		                    ->joinLeft( 'asign_yellowcube_product', 'asign_yellowcube_product.artid = s_articles_details.articleID', [
			                    'lastSent',
			                    'ycSpsDetails',
			                    'ycResponse',
			                    'ycReference',
		                    ...
		                    ->group( 'ordernumber' )
		                    ->joinLeft( 'asign_yellowcube_inventory', 'asign_yellowcube_inventory.articlenr = s_articles_details.ordernumber', [
			                    'ycarticlenr',
			                    'additional'
		                    ] );
		...
		if ( $filters ) {
			foreach ( $filters as $filter ) {
				...
				$select->orWhere( 'asign_yellowcube_inventory.ycarticlenr LIKE ?', $filter["value"] . '%' );
				$select->orWhere( 's_articles_details.instock = ?', $filter["value"] );
				$select->orWhere( 'asign_yellowcube_product.ycReference LIKE ?', '%' . $filter["value"] . '%' );
				$select->orWhere( 's_articles_details.ean LIKE ?', '%' . $filter["value"] . '%' );
		...
		if ( $sort ) {
			...
			switch ( $column ) {
				case 'ordernumber':
					...
					break;
				case 'ycarticlenr':
					$select->order( 'asign_yellowcube_inventory.ycarticlenr ' . $direction );
					break;
				case 'name':
				...
				case 'createDate':
					...
					break;
				case 'ean':
					$select->order( 's_articles_details.ean ' . $direction );
					break;
				default:
					$select->order( 's_articles.name ' . $direction );
			}
	...
	public function saveArticleResponseData( $aResponseData, $artid ) {
		...
		// if present then?
		if ( $iCount ) {
			$sQuery = "update `asign_yellowcube_product` set `lastSent` = 1, `createDate`=NOW(), `ycResponse` = '" . $sData . "'" . $sReference . " where `artid` = '" . $artid . "'";
		} else {
			$sQuery = "insert into `asign_yellowcube_product` set `createDate`=NOW(), `artid` = '" . $artid . "', `lastSent` = 1, `ycResponse` = '" . $sData . "'" . $sReference;
		}
